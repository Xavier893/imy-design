...
const checkForm = () => {
  if (password.value.length === 0 || confirmPassword.value.length === 0) {
    ...
    checkAdminPasswordMatches();
  }
};

const checkAdminPasswordMatches = () => {
  if (password.value === confirmPassword.value) {
    if (password.value.length === 0 || confirmPassword.value.length === 0) {
      errorMessage.value = "Please fill in both passwords.";
      passwordCheck.value = false;
      ...
      toast.add({
        severity: "success",
        summary: "Admin Registration Successful",
        detail: "You will be redirected to the Admin Dashboard shortly...",
        life: 3000,
      });
